...
def export_testcases_to_excel(test_cases_content: str, filename: str = None, jiraTicket: dict = None) -> Union[str, None]:
    if filename is None:
...
def fetch_jira_ticket_description(ticket_id: str) -> str:
...
ZEPHYR_API_KEY = os.getenv("ZEPHYR_API_TOKEN")

print(f"\nZEPHYR_API_KEY: {ZEPHYR_API_KEY}  \n")
print(f"ZEPHYR_BASE_URL: {ZEPHYR_BASE_URL}  \n")


def create_zephyr_testcase(project_key: str, name: str, jira_issue_key: str) -> str:
    ...
    payload = {
        ...
        "name": name,
        "objective": f"Auto-generated test case for JIRA {jira_issue_key}",
        "jiraIssue": jira_issue_key
    }
...
def upload_attachment_to_zephyr(testcase_key: str, filepath: str) -> None:
    url = f"{ZEPHYR_BASE_URL}/testcases/{testcase_key}/attachments"
    headers = {
        "Authorization": f"Bearer {ZEPHYR_API_KEY}"
    }
